...
//TODO AUN NO USO ESTA CLASSSS
class Network1(var activity: AppCompatActivity) {
    init {
     ...
     fun httpRequest(context:Context,url: String,httpResponse: HttpResponse){
        if (hayRed()){
            ...
            val solicitud= StringRequest(Request.Method.GET, url, { response: String ->
                ...
                Toast.makeText(context, "no hay conexion http", Toast.LENGTH_SHORT).show()
            })
            queue.add(solicitud)
            }else{
              //  Mensaje.mensajeError(context,Error.NO_HAY_RED)
            Toast.makeText(context, "COMPRA INTERNET AMIGUITO", Toast.LENGTH_SHORT).show()
            }
    }
